...
if(isset($_POST['id'])&&isset($_POST['kind'])&&isset($_POST['rankway'])){
	...
	if($_POST['kind']==0){
		if($newrank == 'def'){
			...
			$sql = "SELECT * from al_list where i=? ORDER BY $newrank limit 25";
		}
		$sth = $dbh->prepare($sql);
		...
		if($newrank == 'def'){
			...
			$sql = "SELECT * from al_list where i=? and kind=? ORDER BY $newrank limit 25";
		}
		$sth = $dbh->prepare($sql);
